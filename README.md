...
In order to run the project from a local environment we need `node>=16`, `npm>=8` installed on our development machines. We also recommend `yarn>=1.22` to manage dependencies but it's optional. The following steps are using the yarn CLI.
...
```shell
git clone git@github.com:RibonDAO/native-dapp.git
```
...
```shell
yarn start
w for running in web
a for running in android
i for running in ios
```

### 🧪 Test

For running the test suite, use the command above. In this project, we are using react-native-testing-library.
